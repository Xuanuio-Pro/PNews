...
echo "[1/7] Kiểm tra cấu hình Docker Compose..."
docker compose config

echo "[2/7] Build các Docker containers..."
docker compose build

echo "[3/7] Khởi chạy dịch vụ pnews-web..."
docker compose up -d pnews-web

echo "[4/7] Đợi dịch vụ khởi động và kiểm tra Health Endpoint..."
# Đợi tối đa 15s cho web server khởi động và trả về HTTP 200 từ healthcheck
SUCCESS=0
for i in {1..15}; do
...
echo "[5/7] Chạy thử dịch vụ pnews-crawler..."
docker compose run --rm pnews-crawler python -X utf8 main.py

echo "[6/7] Xác minh cơ sở dữ liệu SQLite..."
...
echo "[7/7] Xác minh thư mục logs..."
